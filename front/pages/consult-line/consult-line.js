...
this.setData({collected:!this.data.collected})
  ...
  gettopic1(){
    ...
    wx.request({
      ...
      success: (res) =>  {
        console.log(res)
      this.setData({consultList:res.data.consultList})
      }})},
  gettopic(){
  const token=wx.getStorageSync('acc_token');
  const  collegeId=wx.getStorageSync('real_college_id');
  wx.request({
    ...
    cancleupvote:function (e){
      ...
      wx.request({
        url: 'https://knowclear.top/knowclear/consult-upvote/cancelUpvote/'+this.data.consultList[id1].consultId,
        method:"POST",
        data:{
        },
        header: {
  ...
  loadData:function(tail,callback)
  {
    ...
    wx.request({
     url: 'https://knowclear.top/knowclear/consult/getConsultList/'+real_college_id+'/'+this.data.id+'/'+that.data.pageNum1+'/'+this.data.pape,
      method:"POST",
      data:{},
      header: {       //下面是固定格式，照搬
        'Content-Type': 'application/json',
