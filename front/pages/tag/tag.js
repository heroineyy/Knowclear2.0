...
Page({
  ...
  getLabelTopic() {
    ...
    wx.request({
      ...
      url: api.getUrl("/knowclear/label/selectLabelTopicById/") + that.options.labelId,
      method: 'POST',
      data: {
        label_id: 1
      },
  ...
  getTopicVOList() {
    ...
    wx.request({
      ...
      success: (res) => {
        ...
        // })
        that.setData({
          topicVOList: [...that.data.topicVOList, ...res.data.labelTopics.topicVOList]
        })
      ...
      if (upvote == true || upvote == false) {
        ...
        if (upvote) { //取消点赞
          ...
          wx.request({
            ...
            url: api.getUrl("/knowclear/upvote/cancelUpvote/") + this.data.topicVOList[index].topicId,
            method: 'POST',
            data: {},
            header: { //下面是固定格式，照搬
              'Content-Type': 'application/json',
          ...
          wx.request({
            ...
            url: api.getUrl("/knowclear/upvote/upvoteTopic/") + this.data.topicVOList[index].topicId,
            method: 'POST',
            data: {
  ...
  getAttention(e) {
    ...
    wx.request({
      ...
      url: api.getUrl("/knowclear/collection/selectIsCollected/") + that.options.labelId,
      method: "GET",
      data: {},
      header: { //下面是固定格式，照搬
        'Content-Type': 'application/json',
  ...
  cancelAttention(e) {
    ...
    wx.request({
      ...
      url: api.getUrl("/knowclear/collection/cancelCollectLabel/") + that.options.labelId,
      method: "GET",
      data: {},
      header: { //下面是固定格式，照搬
        'Content-Type': 'application/json',
  ...
  toAttention(e) {
    ...
    wx.request({
      ...
      url: api.getUrl("/knowclear/collection/collectLabel/") + that.options.labelId,
      method: "GET",
      data: {},
      header: { //下面是固定格式，照搬
        'Content-Type': 'application/json',
  ...
  changeTimestamp() {
    ...
    for (var i = 0; i < this.data.topicVOList.length; i++) {
      this.data.aftertime.push(util.getDateDiff(this.data.topicVOList[i].gmt_modified))
    }
    this.setData({
