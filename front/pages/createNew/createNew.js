...
Page({
  ...
  sensitive() {
    ...
    submitEvent:function(event){
      var that=this;
      console.log(event);
      that.setData({mytagName : event.detail.value.tagname});  //得到输入的内容
      that.setData({mytagContent : event.detail.value.tagcontent});
...
releaseYourHouse(){
  ...
  wx.request({
    url: 'https://knowclear.top/knowclear/label/publishChatLabel/0/'+college_id,  
    // url: 'https://knowclear.top/knowclear/label/publishChatLabel/'+that.data.classify+'/'+'1',
    method:"POST",
    data:{
    ...
    success(res){
        ...
        wx.getFileSystemManager().readFile({
          ...
          success: res => { //成功的回调
            ...
            that.setData({mytagPicture : 'data:image/png;base64,' + res.data})
          }
        })
    }
  })
